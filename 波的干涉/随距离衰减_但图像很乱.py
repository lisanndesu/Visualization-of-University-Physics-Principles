...
def update(t):
    ...
    # k = 1 - 1.2*distance/15
    distance1 = np.sqrt((X - source1[0]) ** 2 + (Y - source1[1]) ** 2)
    distance2 = np.sqrt((X - source2[0]) ** 2 + (Y - source2[1]) ** 2)
    z1 = wave_amplitude(X, Y, t, source1)/((1 + distance1)**1.2)
    z2 = wave_amplitude(X, Y, t, source2)/((1 + distance2)**1.2)
    Z =  z1+z2 #if t>distance/v else 0
