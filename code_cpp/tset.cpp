...
class Teacher:virtual public Person{
	private:
		...
		static string clas;
	public:
		Teacher(string _name, string _id, int _age) :Person(_name, _age) {printf("进入teacher构造函数");id = _id; wages = _age*98+(_age+3423)%13*50;}
	...
	do
	{
		...
		if (choice==1)
		{
			...
			do{
				...
				if(choice_tch>=1&&choice_tch<=4)cout<<"操作成功"<<endl;
